...
const renderProducts = () => {
  ...
  if (state.products.length === 0) {
    ...
    const productsHTML = displayProducts
    ...
    deleteBtns.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (productIndex !== -1) {
          ...
          showSnackbar(
            `Product "${deletedProduct.name}" (ID: ${deletedProduct.id}) has been deleted.`
          );
        }
      });
    });
...
const renderCategoryFilters = () => {
  ...
  const categoryButtonsHTML = categories
    .map((category) => createCategoryButtonHTML(category, state.activeCategory))
    .join("");
  ...
  productForm.addEventListener("submit", (e) => {
    ...
    if (state.editProductId) {
      ...
      if (productIndex !== -1) {
        ...
        showSnackbar(
          `Product "${updatedProduct.name}" (ID: ${updatedProduct.id}) has been updated.`
        );
      }
    } else {
      // Add new product
